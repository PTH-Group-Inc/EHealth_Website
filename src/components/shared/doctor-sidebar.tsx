...
import { DOCTOR_MENU_GROUPS, ROUTES, type DoctorMenuGroup } from "@/constants/routes";
...
function getAllHrefs(groups: DoctorMenuGroup[]): string[] {
    const hrefs: string[] = [];
...
// Tính 1 lần ở module scope — DOCTOR_MENU_GROUPS là const, không đổi giữa các render.
// Nếu tính trong component sẽ tạo array mới mỗi render → useCallback/useEffect invalidate → vòng lặp render.
const DOCTOR_ALL_HREFS = getAllHrefs(DOCTOR_MENU_GROUPS);

function isItemActive(href: string, pathname: string, allHrefs: string[]): boolean {
    if (pathname === href) return true;
    if (href === ROUTES.PORTAL.DOCTOR.DASHBOARD) return false;
...
function isChildActive(group: DoctorMenuGroup, pathname: string, allHrefs: string[]) {
...
export function DoctorSidebar() {
    ...
    const tGroupRaw = useTranslations("common.nav.portal.doctorGroup");
    const tItemRaw = useTranslations("common.nav.portal.doctor");

    const allHrefs = DOCTOR_ALL_HREFS;
    const [openGroupKey, setOpenGroupKey] = useState<string | null>(null);

    // Tự mở nhóm chứa route đang active (chỉ phụ thuộc pathname — allHrefs + DOCTOR_MENU_GROUPS là const)
    useEffect(() => {
        for (const g of DOCTOR_MENU_GROUPS) {
            if (g.children && isChildActive(g, pathname, DOCTOR_ALL_HREFS)) {
    ...
    return (
        <aside
            className={`${collapsed ? "w-[72px]" : "w-64"} bg-white dark:bg-[#1e242b] border-r border-[#e5e7eb] dark:border-[#2d353e] flex flex-col h-full shrink-0 z-20 transition-all duration-300`}
        >
            {/* Logo + Toggle */}
                ...
                {!collapsed && (
                    <>
                        ...
                        <div className="flex flex-col flex-1">
                            <h1 className="text-lg font-bold text-[#121417] dark:text-white leading-tight">E-Health</h1>
                            <p className="text-[#687582] dark:text-gray-400 text-xs font-medium">{tNav("doctorTagline")}</p>
                        </div>
                    </>
                )}
                {collapsed && (
            ...
            <div className="p-4 border-t border-[#e5e7eb] dark:border-[#2d353e]">
                <div
                    className={`flex items-center ${collapsed ? "justify-center" : "gap-3"} p-2 rounded-lg hover:bg-[#f1f2f4] dark:hover:bg-gray-800 transition-colors`}
                >
                    <div
                    ...
                    {!collapsed && (
                        <div className="flex flex-col overflow-hidden flex-1 min-w-0">
                            <p className="text-sm font-semibold text-[#121417] dark:text-white truncate">BS. Nguyễn Văn Minh</p>
                            <p className="text-xs text-[#687582] dark:text-gray-400 truncate">Khoa Nội Tổng Quát</p>
                        </div>
                    )}
                    {!collapsed && (
                        <button
                        ...
                        >
                            <span className="material-symbols-outlined text-[#687582] hover:text-red-500 text-[20px]">logout</span>
                        </button>
                    )}
                </div>
            </div>
