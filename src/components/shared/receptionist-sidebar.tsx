...
import { RECEPTIONIST_MENU_GROUPS, ROUTES, type ReceptionistMenuGroup } from "@/constants/routes";
...
function getAllHrefs(groups: ReceptionistMenuGroup[]): string[] {
    const hrefs: string[] = [];
...
// Tính 1 lần ở module scope — RECEPTIONIST_MENU_GROUPS là const, không đổi giữa các render.
const RECEPTIONIST_ALL_HREFS = getAllHrefs(RECEPTIONIST_MENU_GROUPS);

function isItemActive(href: string, pathname: string, allHrefs: string[]): boolean {
    if (pathname === href) return true;
    if (href === ROUTES.PORTAL.STAFF.DASHBOARD) return false;
...
function isChildActive(group: ReceptionistMenuGroup, pathname: string, allHrefs: string[]) {
...
export function ReceptionistSidebar() {
    ...
    const tGroupRaw = useTranslations("common.nav.portal.receptionistGroup");
    const tItemRaw = useTranslations("common.nav.portal.staff");

    const allHrefs = RECEPTIONIST_ALL_HREFS;
    const [openGroupKey, setOpenGroupKey] = useState<string | null>(null);

    // Tự mở nhóm chứa route đang active
    useEffect(() => {
        for (const g of RECEPTIONIST_MENU_GROUPS) {
            if (g.children && isChildActive(g, pathname, RECEPTIONIST_ALL_HREFS)) {
    ...
    return (
        <aside className={`${collapsed ? "w-[72px]" : "w-64"} bg-white dark:bg-[#1e242b] border-r border-[#e5e7eb] dark:border-[#2d353e] flex flex-col h-full shrink-0 z-20 transition-all duration-300`}>
            {/* Logo + Toggle — giữ branding Lễ tân */}
            ...
            <nav className={`flex-1 ${collapsed ? "px-2" : "px-4"} py-2 space-y-0.5 overflow-y-auto`}>
                {RECEPTIONIST_MENU_GROUPS.map((g) => (
                    <SidebarGroupItem
                        key={g.key}
                        group={g}
                        pathname={pathname}
            ...
            {/* User Profile — giữ avatar "LT" + role "Lễ tân" */}
