    ...
    return (
        <div className="p-6 space-y-6">
            <PageHeader
                ...
                subtitle="Quyền, Trình đơn, Quyền API và Phân hệ"
                icon="lock"
            ...
            <div className="bg-white dark:bg-[#1e242b] rounded-2xl border border-[#dde0e4] dark:border-[#2d353e] shadow-sm p-1.5 inline-flex gap-1 flex-wrap">
                {([
                    { key: "permissions", label: "Quyền", icon: "lock" },
                    { key: "menus", label: "Trình đơn", icon: "menu" },
                    { key: "api-permissions", label: "Quyền API", icon: "api" },
                    { key: "modules", label: "Phân hệ", icon: "view_module" },
    ...
    return (
        <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Tổng quyền" value={items.length} icon="lock" color="blue" loading={loading} />
                <StatCard label="Phân hệ" value={Object.keys(byModule).length} icon="view_module" color="violet" loading={loading} />
                <StatCard label="Có mô tả" value={items.filter((p) => p.description).length} icon="description" color="emerald" loading={loading} />
                <StatCard label="Không phân hệ" value={(byModule["Khác"] ?? []).length} icon="help" color="amber" loading={loading} />
            </div>
            ...
            {loading ? (
                ...
                <EmptyState icon="lock" title="Chưa có quyền" description="Hệ thống chưa định nghĩa quyền nào." />
            ) : (
...
function MenusTab() {
    ...
    return (
        <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Tổng trình đơn" value={items.length} icon="menu" color="blue" loading={loading} />
                <StatCard label="Trình đơn cha" value={items.filter((m) => !m.parent).length} icon="folder" color="violet" loading={loading} />
                <StatCard label="Trình đơn con" value={items.filter((m) => m.parent).length} icon="folder_open" color="emerald" loading={loading} />
                <StatCard label="Có biểu tượng" value={items.filter((m) => m.icon).length} icon="emoji_symbols" color="amber" loading={loading} />
            </div>
            ...
            {loading ? (
                ...
                <EmptyState icon="menu" title="Chưa có trình đơn" description="Hệ thống chưa định nghĩa trình đơn nào." />
            ) : (
                <div className="bg-white dark:bg-[#1e242b] rounded-2xl border border-[#dde0e4] dark:border-[#2d353e] shadow-sm overflow-hidden">
                    <table className="w-full text-sm">
                        <thead className="bg-[#f8f9fa] dark:bg-[#13191f] border-b border-[#dde0e4] dark:border-[#2d353e]">
                            <tr>
                                <th className="text-left px-4 py-3 font-semibold text-[#687582]">Biểu tượng</th>
                                ...
                                <th className="text-left px-4 py-3 font-semibold text-[#687582]">Trình đơn cha</th>
    ...
    return (
        <>
            ...
            {loading ? (
                ...
                <EmptyState icon="api" title="Chưa có quyền API" description="Khai báo endpoints để kiểm soát quyền gọi API." />
            ) : (
                <div className="bg-white dark:bg-[#1e242b] rounded-2xl border border-[#dde0e4] dark:border-[#2d353e] shadow-sm overflow-hidden">
                    <table className="w-full text-sm">
                        <tbody>
    ...
    return (
        <>
            ...
            {loading ? (
                ...
                <EmptyState icon="view_module" title="Chưa có phân hệ" description="Phân hệ được định nghĩa từ database, có thể lấy từ danh sách quyền." />
            ) : (
                    ...
                    {items.map((m, idx) => (
                        <div key={m.name || `module-${idx}`} className="bg-white dark:bg-[#1e242b] rounded-2xl border border-[#dde0e4] dark:border-[#2d353e] shadow-sm p-4">
                            <div className="flex items-center justify-between gap-3">
                                ...
                                <div className="font-mono text-sm font-bold text-[#3C81C6]">{m.permissionCount} quyền</div>
                            </div>
                        </div>
                    ))}
                </div>
